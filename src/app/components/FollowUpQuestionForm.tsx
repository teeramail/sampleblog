...
export function FollowUpQuestionForm({ postId, onQuestionAdded }: FollowUpQuestionFormProps) {
  ...
  const createQuestion = api.post.addFollowUpQuestion.useMutation({
    onSuccess: () => {
      ...
      setImageUrls([]);
      setIsExpanded(false);
      toast.success('Follow-up question submitted successfully!');
      if (onQuestionAdded) {
        onQuestionAdded();
      }
    },
  ...
  if (!isExpanded) {
    return (
      <button 
        ...
        className="mt-4 w-full rounded-md border border-blue-300 bg-blue-50 py-3 text-blue-600 hover:bg-blue-100"
      >
  ...
  return (
    <div className="mt-6 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-xl font-semibold">Add a Follow-up Question</h3>
        <button 
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-4">
          <label htmlFor="question-content" className="mb-1 block text-sm font-medium text-gray-700">
        ...
        <button
          ...
          className={`rounded-md bg-blue-600 px-4 py-2 text-white transition hover:bg-blue-700 ${
            (createQuestion.isPending || isUploading) ? 'cursor-not-allowed opacity-70' : ''
          }`}
        >
